...
const CUSTOM_EMAIL_ENDPOINT = ""; // ex.: "/api/send-email" (vazio se não usar)
...
export default function CleaningReportApp() {
  ...
  async function buildInvoicePDF(inv: Invoice) {
    ...
    inv.items.forEach((it) => {
      const descLines = doc.splitTextToSize(it.description || "-", pageWidth - margin*2 - 140);
  ...
  async function downloadInvoicePDF(id: number) {
    ...
    try { pdf.save(filename); } catch { const blobUrl = pdf.output("bloburl"); const a = document.createElement("a"); a.href = blobUrl; a.download = filename; document.body.appendChild(a); a.click(); a.remove(); setTimeout(()=>URL.revokeObjectURL(blobUrl), 5000); }
  }
