...
const [reports, setReports] = useState<Report[]>(() => {
  ...
  const schedulePush = () => {
    ...
    // prepare payload
    const payload = { reports, invoices, presets, banks, updatedAt: Date.now() };
    pushTimer.current = window.setTimeout(() => { pushAll(workspaceId, payload); }, 800);
  };
  useEffect(() => { schedulePush(); /* eslint-disable-next-line react-hooks/exhaustive-deps */ }, [reports, invoices, presets, banks]);
    ...
    (async () => {
      ...
      if (data && (Array.isArray(data.reports) || Array.isArray(data.invoices))) {
        try {
          ...
          if (Array.isArray((data as any).banks)) { setBanks((data as any).banks); localStorage.setItem("cleaning_banks_v1", JSON.stringify((data as any).banks)); }
        } catch {}
      }
    })();
